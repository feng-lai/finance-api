...
class Places
{
    ...
    public function getList($map=[]){
        ...
        $list = $model->where($map)->json(['disabled'])->order('create_time','desc')->paginate($page_param);
        foreach ($list as $k=>$v){
            $arr = [];
    ...
    public function updateData($uuid,$save_data){
        if($save_data['disabled']){
            $arr = [];
            foreach (json_decode($save_data['disabled'],true) as $key=>$val){
                $arr[$key]['from'] =  date('Y-m-d H:i',strtotime($val['from']));
                $arr[$key]['to'] =  date('Y-m-d H:i',strtotime($val['to']));
            }
            $save_data['disabled'] = json_encode($arr);
        }
        $model = new PlacesModel();
        // 启动事务 
        Db::startTrans();
    ...
    public function lock($data){
        ...
        try{
            foreach($data as $v){
                $arr = [];
                foreach ($v['disabled'] as $key=>$val){
                    $arr[$key]['from'] =  date('Y-m-d H:i',strtotime($val['from']));
                    $arr[$key]['to'] =  date('Y-m-d H:i',strtotime($val['to']));
                }
                $save_data['disabled']=json_encode($arr);
                if ( $model->where('uuid',$v['uuid'])->update($save_data) === false ) {
                    throw new \Exception("保存失败");
                }
            }
            // 更新成功 提交事务
