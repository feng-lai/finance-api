...
class Activity
{
    ...
    public function getList($map=[]){
        ...
        foreach($list as $k=>$v){
            ...
            $list[$k]->sign_start_time = date('Y/m/d H:i',strtotime($v->sign_start_time));
            $list[$k]->sign_end_time = date('Y/m/d H:i',strtotime($v->sign_end_time));
        }
        return $list;
    }
    ...
    public function read($uuid){
        ...
        $data['sign'] = SignsModel::where('activity_uuid',$uuid)->count();
        ...
        if(input('user_uuid')){
            $data['sign_time'] = SignsModel::where(['user_uuid'=>input('user_uuid'),'activity_uuid'=>$uuid])->value('sign_time');
        }else{
            $data['sign_time'] = '';
        }
        $data['start_time'] = date('Y/m/d H:i',strtotime($data['start_time']));
        $data['end_time'] = date('Y/m/d H:i',strtotime($data['end_time']));
        $data['sign_start_time'] = date('Y/m/d H:i',strtotime($data['sign_start_time']));
        $data['sign_end_time'] = date('Y/m/d H:i',strtotime($data['sign_end_time']));
        return $data;
    }
